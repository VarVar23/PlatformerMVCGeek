...
{
    public class SpriteAnimatorControllers : IDisposable
    {
        ...
        public SpriteAnimatorControllers(SpriteAnimatorConfig config)
        {
            _config = config;
        }
        ...
        public void StartAnimation(SpriteRenderer spriteRenderer, AnimState track, bool loop, float speed)
        {
            if(_activeAnimation.TryGetValue(spriteRenderer, out var animation))
            {
                ...
                if(animation.Track != track)
                {
                    ...
                    animation.Sprites = _config.Sequens.Find(sequence => sequence.Track == track).Sprites;
                    animation.Counter = 0;
                }
            }
            else
            {
                ...
                {
                    Track = track,
                    Sprites = _config.Sequens.Find(sequence => sequence.Track == track).Sprites,
                    Loop = loop,
                    Speed = speed
                });
            }
        ...
        public void Update()
        {
            foreach(var anim in _activeAnimation)
            { 
                anim.Value.Update();
